...
from src.Vevmodel.vc.fmt_model import FlowMatchingTransformer
from src.Vevmodel.codec.repcodec_model import RepCodec
from src.Vevmodel.codec.melspec import MelSpectrogram
from src.Vevmodel.codec.vocos import Vocos
from src.Vevmodel.utils.util import load_config
...
class VevoInferencePipeline:
    ...
    def inference_fm(
        ...
        try:
            timbre_src_speech, timbre_src_speech24k, timbre_src_speech16k = load_wav(
            src_wav_tok, self.device
        )
            src_hubert_codecs, _ = self.extract_hubert_codec(
                self.content_style_tokenizer, timbre_src_speech16k, duration_reduction=False
            )
        except:
            src_hubert_codecs = src_wav_tok
